...
Рабочая директория: `SDTWork/` (не из sub-директорий). `SDTWork/memory/` = симлинк на auto-memory.
...
3. Перенести в `SDT` + commit `feat: promote S-NN from staging`
...
Авторское → `extensions/`. Платформенное → `SDT`, затем `update.sh`.

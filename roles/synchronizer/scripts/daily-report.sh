...
LOG_DIR="/Users/alfa/logs/synchronizer"
STRATEGY_DIR="SDTWork/DS-strategy"

# Agent Workspace: если существует — отчёты идут туда
AGENT_WORKSPACE="SDTWork/DS-agent-workspace"
...
STRATEGIST_LOG="/Users/alfa/logs/strategist/$DATE.log"
...
"
        if echo "$warnings" | grep -q "push failed" 2>/dev/null; then
            report+="- **push failed:** Mac был оффлайн. Запусти \`cd SDTWork/DS-strategy && git pull --rebase && git push\`
"
        fi
    else
        report+="

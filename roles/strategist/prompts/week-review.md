...
Источник сценария: SDTWork/PACK-digital-platform/pack/digital-platform/02-domain-entities/DP.ROLE.012-strategist/scenarios/scheduled/03-week-review.md

## Контекст

- **WeekPlan:** SDTWork/DS-strategy/current/WeekPlan W*.md
- **Шаблон:** SDTWork/PACK-digital-platform/pack/digital-platform/02-domain-entities/DP.ROLE.012-strategist/templates/reviews/weekly-review.md
...
```bash
# Для КАЖДОГО репо в SDTWork/:
git -C SDTWork/<repo> log --since="last monday 00:00" --until="today 00:00" --oneline --no-merges
```

- Пройди по ВСЕМ репозиториям в `SDTWork/`
...
1. Переключись на **роль Автора (R4)** и на основе секции «Итоги W{N}» в WeekPlan сформируй пост для клуба.
   ...
   **Обязательно прочитай** `SDTWork/DS-Knowledge-Index/CLAUDE.md` — полные инструкции роли Автора:
...
2. Создай файл `SDTWork/DS-Knowledge-Index/docs/{YYYY}/{YYYY-MM-DD}-week-review-w{N}.md`
...
4. Обнови `SDTWork/DS-Knowledge-Index/docs/README.md` — добавь строку в начало текущего месяца
5. Закоммить `DS-Knowledge-Index` (git add docs/ && git commit)

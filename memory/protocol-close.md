...
for repo in $(ls SDTWork/); do
  if [ -d SDTWork/$repo/.git ]; then
    commits=$(git -C SDTWork/$repo log --since="today 00:00" --oneline --no-merges 2>/dev/null)
...
SDTWork/DS-IT-systems/DS-ai-systems/synchronizer/scripts/day-close.sh
```
...
- Есть строки со статусом `validated`? → выполнить чеклист промоции (убрать авторские константы → SDT → commit `feat: promote S-NN`)

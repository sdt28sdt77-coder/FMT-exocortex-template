...
# Configures a forked SDT: placeholders, memory, launchd, DS-strategy
#
# Usage:
...
if [ ! -f "$TEMPLATE_DIR/CLAUDE.md" ] || [ ! -d "$TEMPLATE_DIR/memory" ]; then
    echo "ERROR: This script must be run from the root of SDT."
    echo "  Expected: $TEMPLATE_DIR/CLAUDE.md and $TEMPLATE_DIR/memory/"
    echo ""
    echo "  Steps:"
    echo "    gh repo fork TserenTserenov/SDT --clone"
    echo "    cd SDT"
    echo "    bash setup.sh"
    exit 1
fi
...
if $DRY_RUN; then
    ...
    echo "    SDTWork → $WORKSPACE_DIR"
    echo "    claude → $CLAUDE_PATH"
    echo "    SDTWork → $CLAUDE_PROJECT_SLUG"
    echo "    4 → $TIMEZONE_HOUR"
    echo "    4:00 UTC → $TIMEZONE_DESC"
    echo "    /Users/alfa → $HOME_DIR"
else
    find "$TEMPLATE_DIR" -type f \( -name "*.md" -o -name "*.json" -o -name "*.sh" -o -name "*.plist" -o -name "*.yaml" -o -name "*.yml" \) | while IFS= read -r file; do
        sed_inplace \
            ...
            -e "s|SDTWork|$WORKSPACE_DIR|g" \
            -e "s|claude|$CLAUDE_PATH|g" \
            -e "s|SDTWork|$CLAUDE_PROJECT_SLUG|g" \
            -e "s|4|$TIMEZONE_HOUR|g" \
            -e "s|4:00 UTC|$TIMEZONE_DESC|g" \
            -e "s|/Users/alfa|$HOME_DIR|g" \
            "$file"
    done
...
# === 1b. Rename repo (if name differs from SDT) ===
